...
const CreatePostModal = ({ isOpen, onClose, onCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.tripId) payload.append('tripId', formData.tripId);
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="bg-[#0A1622] border border-white/10 rounded-[32px] shadow-2xl w-full max-w-2xl">
              <div className="flex items-center justify-between p-8 pb-4 border-b border-white/5">
                <h2 className="text-2xl font-heading font-bold text-secondary-bg">Create Post</h2>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="flex w-full rounded-[16px] border border-white/10 bg-white/5 px-4 py-3.5 text-sm text-secondary-bg shadow-sm transition-all placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-accent-blue/50 focus:border-accent-blue resize-none h-28"
                    required
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="tripId">Attach Trip</Label>
                    <select
                      id="tripId"
                      name="tripId"
                      value={formData.tripId}
                      onChange={handleChange}
                      className="flex w-full rounded-[16px] border border-white/10 bg-white/5 px-4 py-3.5 text-sm text-secondary-bg shadow-sm transition-all focus:outline-none"
                    >
                      <option value="" className="bg-[#0A1622]">None</option>
                      {trips.map(trip => (
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      {['PUBLIC', 'FRIENDS', 'PRIVATE'].map(option => (
                        <option key={option} value={option} className="bg-[#0A1622]">{option}</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                <div>
                  <label className="flex items-center gap-3 text-sm text-neutral-text cursor-pointer">
